...
const ProductWorksheetItem = (props) => {
  ...
  const handleSubtractItem = () => {
    if (quantity !== 0) {
      ...
      props.subtractItem(props.price)
    }
  }

  const handleAddItem = (subTotal) => {
    let value = subTotal + props.price
    setQuantity(quantity + 1);
    setSubTotal(value)
    props.addItem(props.price)
  }
      ...
      <tr scope="row" className={quantity > 0 ? 'table-active' : ''}>
        ...
        <td>
          <div className="d-flex flex-column flex-md-row align-items-center action-btn">
            ...
            <i role='button' className="fa-solid fa-plus ms-md-2 d-block" onClick={() => handleAddItem(subTotal)}></i>
          </div>
        </td>
